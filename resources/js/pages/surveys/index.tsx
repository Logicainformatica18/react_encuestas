...
type Survey = {
    ...
    state?: string;
    quanty?: number;
    contract_end_type?: 'by_day_and_months' | 'by_days' | 'fixed';
contract_duration_months?: number;
contract_end_day?: number;
contract_duration_days?: number;
contract_end_date?: string;
contract_end_date_calculated?: string;
                    ...
                    <table className="min-w-full divide-y divide-gray-200 bg-white dark:bg-black shadow-md rounded">
                        <thead className="bg-gray-100 dark:bg-gray-800">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-black dark:text-white">Tipo vencimiento</th>
<th className="px-4 py-2 text-black dark:text-white">Meses</th>
<th className="px-4 py-2 text-black dark:text-white">Día fijo</th>
<th className="px-4 py-2 text-black dark:text-white">Días</th>
<th className="px-4 py-2 text-black dark:text-white">Fecha fin fija</th>
<th className="px-4 py-2 text-black dark:text-white">Fecha fin calculada</th>
                        ...
                        <tbody>
                            {surveys.map((s) => (
                                <tr key={s.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700 text-black dark:text-white">
                                    ...
                                    <td className="px-4 py-2">{s.contract_end_type ?? '-'}</td>
<td className="px-4 py-2">{s.contract_duration_months ?? '-'}</td>
<td className="px-4 py-2">{s.contract_end_day ?? '-'}</td>
<td className="px-4 py-2">{s.contract_duration_days ?? '-'}</td>
<td className="px-4 py-2">{s.contract_end_date ?? '-'}</td>
<td className="px-4 py-2">
  {s.contract_end_date_calculated
    ? new Date(s.contract_end_date_calculated).toLocaleDateString('es-PE', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
      })
